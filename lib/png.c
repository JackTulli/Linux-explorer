 ...
 * filter byte). `bpp` is bytes per pixel, rounded up. The filter is looked
 * at once a row and each has a straight loop of its own: asked byte by
 * byte, with the neighbours tested for every one, it cost a 4K picture
 * 10-20 ms more. The first row has nothing above it, which the
 * specification reads as a row of zeros; a filter above 4 does nothing. */
static void unfilter(unsigned char *raw, int h, int bpp, int stride)
{
    ...
    for (int y = 0; y < h; y++) {
        ...
        switch (row[0]) {
        case 1:                                                  /* Sub */
            for (i = bpp; i < stride; i++) cur[i] += cur[i - bpp];
            break;
        case 2:                                                  /* Up */
            if (prev) for (i = 0; i < stride; i++) cur[i] += prev[i];
            break;
        case 3:                                                  /* Average */
            if (!prev) {
                for (i = bpp; i < stride; i++) cur[i] += cur[i - bpp] >> 1;
                break;
            }
            for (i = 0; i < bpp && i < stride; i++) cur[i] += prev[i] >> 1;
            for (; i < stride; i++) cur[i] += (cur[i - bpp] + prev[i]) >> 1;
            break;
        case 4:                     /* Paeth: Sub on the first row, Up at a row's start */
            if (!prev) {
                for (i = bpp; i < stride; i++) cur[i] += cur[i - bpp];
                break;
            }
            for (i = 0; i < bpp && i < stride; i++) cur[i] += prev[i];
            for (; i < stride; i++) cur[i] += paeth(cur[i - bpp], prev[i], prev[i - bpp]);
            break;
...
unsigned char *w2k_png_load(const char *path, int *out_w, int *out_h)
{
    ...
    unfilter(raw, h, channels, stride);

    /* The colour type, likewise, once a row rather than once a pixel. */
    for (int y = 0; y < h; y++) {
        ...
        switch (color) {
        case 0:
            for (x = 0; x < w; x++, o += 4, s++) { o[0] = o[1] = o[2] = s[0]; o[3] = 255; }
            break;
        case 2:
            for (x = 0; x < w; x++, o += 4, s += 3) {
        ...
        case 3:
            ...
            break;
        case 4:
            for (x = 0; x < w; x++, o += 4, s += 2) { o[0] = o[1] = o[2] = s[0]; o[3] = s[1]; }
            break;
        default:                                         /* 6: already RGBA */
            memcpy(o, s, (size_t)w * 4);
            break;
        }
    }
    free(raw);
...
/* One chunk at `p`: length, type, data, CRC. Returns the bytes it took. */
static size_t chunk(unsigned char *p, const char *type, const unsigned char *data, size_t n)
{
    put32(p, (unsigned long)n);
    memcpy(p + 4, type, 4);
    if (n) memcpy(p + 8, data, n);
    unsigned long crc = crc32(0L, Z_NULL, 0);
    crc = crc32(crc, p + 4, (uInt)(4 + n));
    put32(p + 8 + n, crc);
    return 12 + n;
}

/* The file's bytes for an RGBA picture, malloc'd; the clipboard hands
 * these out as image/png. NULL on failure. The chunks are put together in
 * memory: they went through a tmpfile() and back, which also failed the
 * copy and the save outright wherever the temporary directory could not
 * be written. */
unsigned char *w2k_png_encode(const unsigned char *rgba, int w, int h, size_t *out_n)
{
    ...
    size_t n = 8 + (12 + 13) + (12 + (size_t)zn) + 12;
    unsigned char *out = malloc(n), *p = out;
