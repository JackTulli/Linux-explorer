...
#define S(...) (const Tint[]){ __VA_ARGS__ }
    ...
    /* The classic look with the lights off: dark greys for the face,
     * menus and windows, light text, the navy title bar kept but its
     * gradient and the desktop dimmed to match. */
    { "Windows Classic Dark", S({C_FACE,64,64,64},{C_LIGHT,80,80,80},{C_HILIGHT,112,112,112},
        {C_SHADOW,32,32,32},{C_DKSHADOW,0,0,0},{C_TEXT,230,230,230},{C_GRAYTEXT,140,140,140},
        {C_WINDOW,30,30,30},{C_WINDOWTEXT,230,230,230},{C_WINDOWFRAME,0,0,0},
        {C_ACTIVETITLE,10,36,106},{C_ACTIVETITLE2,48,96,160},{C_TITLETEXT,255,255,255},
        {C_INACTIVETITLE,56,56,56},{C_INACTIVETITLE2,96,96,96},{C_INACTIVETITLETEXT,180,180,180},
    ...
    { "Modern Dark", S({C_FACE,32,32,32},{C_LIGHT,32,32,32},{C_HILIGHT,72,72,72},
        ...
        {C_ACTIVETITLE,38,38,38},{C_ACTIVETITLE2,26,26,26},{C_TITLETEXT,255,255,255},
        {C_INACTIVETITLE,32,32,32},{C_INACTIVETITLE2,28,28,28},{C_INACTIVETITLETEXT,128,128,128},
...
static void read_monitors(void)
{
    ...
    for (int i = 0; i < nmons; i++) {
        ...
        /* Under desktop or supersampled scaling the transform xrandr
         * shows is the render scale over the wanted one, so the wanted
         * scale is read back from it. */
        if (w2k_scale_mode != SCALE_XRANDR && m->cur_mode >= 0 && m->w > 0) {
            ...
            if (mw > 0) {
                ...
                m->scale = 100;
                for (int k = 0; k < NSCALES; k++)
                    if (abs(want - scales[k]) <= 3) m->scale = scales[k];
            }
        }
...
{
    int wants[8], primary = -1;
    for (int i = 0; i < nmons && i < 8; i++) {
        wants[i] = mons[i].want_scale > 0 ? mons[i].want_scale : 100;
        if (mons[i].want_primary && primary < 0) primary = i;
    }
    return w2k_scale_render(scale_method, wants, nmons < 8 ? nmons : 8, primary < 0 ? 0 : primary);
}
...
static void pending_size(const Monitor *m, int *w, int *h)
{
     ...
     * the desktop renders at 200% and the panel shows it shrunk. */
    double f = w2k_scale_transform(scale_method, pending_render(), m->want_scale, NULL);
    *w = (int)(mw * f + 0.5);
    *h = (int)(mh * f + 0.5);
}
...
static void rate_scale_args(const Monitor *m, char *out, int n)
{
    ...
    char sc[72];
    /* The transform for the method: see w2k_scale_transform(). */
    const char *flt;
    double f = w2k_scale_transform(scale_method, pending_render(), m->want_scale, &flt);
    if (f > 1.0001 || f < 0.9999)
        snprintf(sc, sizeof sc, " --scale %.4fx%.4f --filter %s", f, f, flt);
    else
...
static void paint(W2kWin *w, Drawable d)
{
    ...
    case 2: {
        ...
        if (valid) {
            ...
            const char *how = "";
            int want = mons[cur].want_scale, render = pending_render();
            if (want == 100 && render == 100)
                how = "";
...
static void record_monitors(void)
{
    ...
    for (int i = 0; i < nmons && w2k_monitor_cfg_n < 8; i++) {
        ...
        /* What the monitor should show; the mode decides how the
         * transform and the desktop's render scale produce it. */
